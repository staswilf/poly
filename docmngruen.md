...
Хранилище двуязычной конфигурации **ДокументооборотКОРПРусскийИАнглийскийИнтерфейс** обновляем, как обычно, после захвата всех объектов, обновлением на файлы поставки **DocumentManagementBilingual**. Занимает это 3-4 часа. При этом мы анализируем объекты, измененные по отношению к старой конфигурации поставщика, в т.ч.:
...
* Разбираемся с новыми и измененными схемами компоновки данных отчетов. **Самый трудоемкий пункт**.
  * Новые - переводим вручную. При этом:
    * В наборах данных переводим синонимы полей, которые платформа не смогла привязать к синонимам свойств объектов метаданных (те, что остались на русском)
    * Даем англоязычные заголовки вычисляемым полям и параметрам
    * Переводим синонимы вариантов отчета
    * Переводим, при наличии, собственные заголовки таблиц и диаграмм на закладке "Настройка"
...
* Разбираемся с измененными макетами типа "Табличный документ"
  ...
  * Удобно видеть изменения, сравнивая старую русскоязычную конфигурацию с новой
